...
export default function EditPageEditor() {
  ...
  // Load old page details, parent options, and settings on mount
  useEffect(() => {
    async function loadPageDetails() {
      ...
      try {
        ...
        if (settingsData.success && settingsData.settings) {
          setIsSeoPluginEnabled(settingsData.settings.plugin_seo_enabled !== 'false');
          setIsGrapesjsEnabled((settingsData.settings.plugin_lexi_page_builder_enabled ?? settingsData.settings.plugin_grapesjs_enabled) !== 'false');
        }

        // 1. Fetch all parent page options
        const parentRes = await fetch('/api/posts?type=PAGE');
        const parentData = await parentRes.json();
        if (parentData.success) {
          setParentOptions(parentData.posts || []);
        }

        // 2. Fetch specific page detail
        ...
        if (data.success && data.post) {
          ...
          setPostStatus(data.post.status === 'PUBLISHED' ? 'published' : 'draft');
          setVisibility(data.post.visibility || 'PUBLIC');
          setPublishedAt(data.post.publishedAt || new Date().toISOString());
          setParentId(data.post.parentId || null);
          
          if (data.post.featuredImage) {
            setFeaturedImage(data.post.featuredImage.url);
            setFeaturedImageId(data.post.featuredImage.id);
          }
          ...
          setGrapesjsData(data.post.builderData || '');
          setPageLayout(data.post.pageLayout || 'THEME_DEFAULT');
          setContentWidthSetting(data.post.contentWidth || 'BOXED');
          setContentMaxWidth(data.post.contentMaxWidth || '1200px');
        ...
        alert('Lỗi khi kết nối cơ sở dữ liệu!');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // Auto Save effect
  useEffect(() => {
    if (!hasUnsavedChanges || !title.trim() || isLoading) return;
  ...
  }, [hasUnsavedChanges, title, slug, content, parentId, featuredImageId, visibility, publishedAt, seoTitle, seoDescription, seoKeywords, isLoading, grapesjsData]);
  ...
  const autoSaveToServer = async () => {
    ...
    try {
      const nextStatus = postStatus === 'published' ? 'PUBLISHED' : 'DRAFT';
      const payload = {
        id: Number(pageId),
  ...
  const savePost = async (isPublish: boolean) => {
    ...
    const nextStatus = isPublish || postStatus === 'published' ? 'PUBLISHED' : 'DRAFT';
    ...
    try {
      ...
      if (data.success) {
        ...
        setHasUnsavedChanges(false);
        setLastSavedAt(new Date());
        const savedStatus = data.post.status === 'PUBLISHED' ? 'published' : 'draft';
        setPostStatus(savedStatus);
        alert(savedStatus === 'published' ? 'Đã cập nhật trang thành công!' : 'Đã cập nhật bản nháp thành công!');
        // Delay reload to let React flush hasUnsavedChanges=false and remove beforeunload listener
  ...
  // Safe recursion to prevent a page from being its own parent or child's parent
  const filteredParentOptions = parentOptions.filter(p => {
    // 1. Cannot be the page itself
    if (p.id === Number(pageId)) return false;
    // 2. Cannot be a direct child of the page itself (helps avoid simple circular references)
    if (p.parentId === Number(pageId)) return false;
    return true;
  });

  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px] text-slate-400 font-medium text-xs animate-pulse">
        Đang tải dữ liệu trang tĩnh...
      </div>
    );
  }
  ...
  return (
    <CapabilityGuard capability="edit_pages">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/pages" className="p-2 bg-white rounded-lg border border-slate-200 text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-colors shadow-sm cursor-pointer animate-fade-in">
          <ArrowLeft size={16} />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Chỉnh sửa Trang Tĩnh</h1>
          <p className="text-xs text-slate-400 font-medium">Chỉnh sửa cấu trúc & nội dung trang tĩnh</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            <div>
              ...
              <input 
                type="text" 
                placeholder="Nhập tiêu đề trang tĩnh..."
                value={title}
            ...
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 outline-none transition-all font-semibold text-brand-600"
              />
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                {isGrapesjsEnabled && (
                  <button
                    type="button"
                    onClick={() => { window.location.href = `/admin/pages/builder/${pageId}`; }}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            >
              <Save size={15} />
              {isPublishing ? 'Đang cập nhật...' : postStatus === 'published' ? 'Cập nhật' : 'Xuất bản'}
            </button>
          </div>
