  ...
  return <>
    ...
    <aside id="mobile-navigation" className={`ezi-mobile-drawer ${mobileOpen?'is-open':''}`} aria-hidden={!mobileOpen}>
      ...
      <div className="ezi-mobile-scroll"><form action="/tim-kiem" method="get" className="ezi-mobile-search"><Search size={17}/><input name="q" placeholder="Tìm kiếm..." aria-label="Từ khóa tìm kiếm"/><button type="submit">Tìm</button></form>
        ...
        <nav aria-label="Điều hướng mobile" className="ezi-mobile-menu">{mobileMenu.map(item=><div key={item.id} className="ezi-mobile-menu-group"><div className="ezi-mobile-menu-row"><Link href={item.url||'#'} onClick={()=>!item.children.length&&setMobileOpen(false)}>{item.label}</Link>{item.children.length>0&&<button type="button" aria-label={`Mở mục ${item.label}`} aria-expanded={!!expanded[item.id]} onClick={()=>setExpanded(value=>({...value,[item.id]:!value[item.id]}))}><ChevronDown className={expanded[item.id]?'is-rotated':''} size={17}/></button>}</div>{item.children.length>0&&<div className={`ezi-mobile-submenu ${expanded[item.id]?'is-open':''}`}>{item.children.map(child=><div key={child.id}><Link href={child.url||'#'} onClick={()=>setMobileOpen(false)}>{child.label}</Link>{child.children.map(grand=><Link key={grand.id} className="ezi-mobile-grandchild" href={grand.url||'#'} onClick={()=>setMobileOpen(false)}>{grand.label}</Link>)}</div>)}</div>}</div>)}</nav>
      </div>
      <a className="ezi-mobile-call" href={`tel:${(settings.contact_hotline_1||settings.site_phone||settings.footer_phone||'').replace(/[^0-9+]/g,'')}`}><Phone size={17}/> Gọi tư vấn ngay</a>
    </aside>
  </>;
}
