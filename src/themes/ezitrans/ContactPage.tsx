...
export default function ContactPage({post,settings={}}:{post:any;settings?:Record<string,string>}) {
 const phone=settings.contact_hotline_1 || settings.contact_phone || settings.site_phone || settings.footer_phone || '';
 const email=settings.site_email || settings.footer_email || settings.contact_email || '';
 const address=settings.contact_address || settings.site_address || settings.footer_address || '';
 return (
     ...
     <div className="ezi-contact-grid-simple">
      ...
      <section className="ezi-contact-info-simple">
       ...
       <div className="ezi-contact-details-list">
        {phone && <div className="ezi-contact-detail-item">
         <div className="ezi-contact-icon-circle"><Phone size={18}/></div>
         <div><span>Hotline & Zalo</span><strong><a href={`tel:${phone.replace(/[^\d+]/g,'')}`}>{phone}</a></strong></div>
        </div>}
        {email && <div className="ezi-contact-detail-item">
         <div className="ezi-contact-icon-circle"><Mail size={18}/></div>
         <div><span>Email</span><strong><a href={`mailto:${email}`}>{email}</a></strong></div>
        </div>}
        {address && <div className="ezi-contact-detail-item">
         <div className="ezi-contact-icon-circle"><MapPin size={18}/></div>
         <div>
          <span>Văn phòng</span><strong>{address}</strong>
          {settings.contact_map && <a className="ezi-contact-map-link" href={settings.contact_map} target="_blank" rel="noreferrer">Mở Google Maps →</a>}
         </div>
        </div>}
       </div>
      </section>
