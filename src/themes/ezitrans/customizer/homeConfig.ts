...
export function resolveTemplate(value:string,settings:Record<string,string>){return text(value,'').replaceAll('{site_name}',settings.site_title||'Ezitrans').replaceAll('{phone}',settings.footer_phone||'').replaceAll('{email}',settings.footer_email||'').replaceAll('{page_url}',settings.site_url||'')}
